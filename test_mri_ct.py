...
for i in range(0, 10):
    ...
    path1 = os.path.join("./test_imgs_mri_ct/",'MRI'+ str(i + 1) + ".png")
    path2 = os.path.join("./test_imgs_mri_ct/", 'CT'+str(i + 1) + ".png")
    ...
    with torch.no_grad():
        ...
        fuse_img = fusion_test(img1,img2,rule='add')

    fuse_img = np.squeeze(fuse_img.cpu().data[0].numpy() * 255.0, 0).astype('uint8')
    fuse_img = Image.fromarray(fuse_img).convert('L')

    image_path = os.path.join("./results_mri_ct/", str(i+1) + ".png")
    fold_path = os.path.join("./results_mri_ct/")
