...
class SimpleKernel2dFn(Function):
    @staticmethod
    def forward(ctx, input, kernel_size, stride, padding, dilation, channel_wise):
        ...
        if not channel_wise:
            cols = cols.sum(dim=1, keepdim=True)
        output = cols
...
def Mypackernel2d(input, mask=None, kernel_size=0, stride=1, padding=0, output_padding=0, dilation=1,
                kernel_type='gaussian', smooth_kernel_type='none', smooth_kernel=None, inv_alpha=None, inv_lambda=None,
                channel_wise=False, normalize_kernel=False, transposed=False, native_impl=True):
    kernel_size = _pair(kernel_size)
    dilation = _pair(dilation)
    padding = _pair(padding)
    output_padding = _pair(output_padding)
...
class PacConv2d(_PacConvNd):
    ...
    def compute_kernel(self, input_for_kernel, input_mask=None):
        return Mypackernel2d(input_for_kernel, input_mask,
